...
struct client {                     // Everything we know about a window.
    ...
    uint8_t workspace;               // The workspace this window is on.
...
uint8_t curws = 1;                  // Current workspace.
...
bool wsshow[WORKSPACES]={true,true,false,false,false,false,false,false,false,false};
...
static void group_hide(const uint32_t ws);
static void group_hide_all();
static void group_show(const uint32_t ws);
static bool hidden(struct client *win);
static void togglegroups();
#include "config.h"

static bool    group_windows = GROUP_WINDOWS;
...
void group_hide_all() { for (int i = 0; i < WORKSPACES; ++i) group_hide(i); }
...
void group_hide(const uint32_t ws)// Hide workspace ws
{
    int nows = WORKSPACE_DEFAULT;

    // Change workspace to the default: WORKSPACE_DEFAULT
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_current_desktop], XCB_ATOM_CARDINAL, 32, 1, &nows);
    struct client *client;
    ...
    // Put workspace state in 'hidden'
    wsshow[ws] = false;
    curws = nows;
}

void group_show(const uint32_t ws)// Show workspace ws
{
    // Set ws as the current workspace
    xcb_change_property(conn, XCB_PROP_MODE_REPLACE, screen->root, ATOM[atom_current_desktop], XCB_ATOM_CARDINAL, 32, 1,&ws);
    struct client *client;
    for (struct item *item = wslist[ws]; item != NULL; item = item->next) {
        /* Go through list of current ws. Map everything that's on the workspace */
        client = item->data;
        if (!client->fixed && !client->iconic) xcb_map_window(conn, client->id);
    }
    // Put workspace state in 'shown'
    wsshow[ws] = true;
    curws = ws;

    // focus a window in the shown desktop (if no other window is focused)
    if (NULL == focuswin && NULL != wslist[curws]) focusnext_helper(true);
}

bool hidden(struct client *win)// Tell wether a window is hidden or not
{
    return !wsshow[win->workspace];
}
...
void togglegroups()
{
    ...
    group_windows = (group_windows) ? false : true;
...
void changeworkspace_helper(const uint32_t ws)// Toggle the state (shown/hidden) of workspace ws
{
    if (group_windows) {
        // Don't do anything to default workspace
        if (ws == WORKSPACE_DEFAULT) return;

        // If the group has windows in it, and is shown, hide it. If not, make it current
        (wslist[ws] && wsshow[ws]) ? group_hide(ws) : group_show(ws);
    } else if (ws != curws) {
        group_hide(curws);
        group_show(ws);
    }
...
{
    if (NULL == focuswin||focuswin->fixed) return;
    delfromworkspace(focuswin, focuswin->workspace);
    addtoworkspace(focuswin, arg->i);
    if (!wsshow[arg->i]) {// Hide window if group is not shown
...
{                                   // Change focus to next in window ring.
    ...
    if (NULL == winlist) return;
    /* If we currently have no focus focus first in list. */
    if (NULL == focuswin || NULL == focuswin->winitem){
        client  = winlist->data;
        while (client->iconic==true || hidden(client)) {
            client = client->winitem->next->data;
            if (client->winitem->next== NULL) break;
        }
    } else {
        if (arg) {
            if (NULL == focuswin->winitem->prev) {
                /* We were at the head of list. Focusing on last window in list unless we were already there.*/ 
                client = wslist[curws]->data;
                /* Go to the end of the list */
                while( client->winitem->next!=NULL )
                    client = client->winitem->next->data;
                /* walk backward until we find a windows that isn't iconic */
                while(client->iconic==true)
                    client = client->winitem->prev->data;
            }
            else
                if(focuswin!=wslist[curws]->data) {
                    client = focuswin->winitem->prev->data;
                    while (client->iconic == true && client->winitem->prev!=NULL)
                        client = client->winitem->prev->data;
            ...
            if (NULL == focuswin->winitem->next) {
                ...
                client = wslist[curws]->data;
                while(client->iconic && client->winitem->next!=NULL)
                    client = client->winitem->next->data;
            }
            else {
                client = focuswin->winitem->next->data;
                while (client->iconic==true && client->winitem->next!=NULL)
                    client = client->winitem->next->data;
    ...
    if (NULL != client && focuswin != client && !hidden(client)) {
        raisewindow(client->id);
...
{                                   // Move window win as a result of pointer motion to coordinates rel_x,rel_y.
    if(focuswin==NULL||hidden(focuswin))return;
